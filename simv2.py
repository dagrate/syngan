...
# In[00]: Global Variables Definitions
...
df_attack_norm = pd.read_csv('./data/df_attack_norm.csv',sep=',', header=0)
lbl = df_attack_norm.columns
# In[3]: build GP-WGAN and generate adversarial samples
#
print("GRADIENT_PENALTY_WEIGHT:", GRADIENT_PENALTY_WEIGHT)
...
generated = pd.DataFrame(generated, columns=lbl[:-1])
...
for col in range(0, 42):
    print(lbl[col], np.unique(df_attack_norm.iloc[:,col]))
...
# In[2]:
#df = pd.read_csv('Monday-WorkingHours.pcap_ISCX.csv', sep=",", header=0, low_memory=False)
#print(df)
#
